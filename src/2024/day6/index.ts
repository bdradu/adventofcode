...
const printMapPath = (map: string[][], path: Map<string, string[]>) => {
  const mapString = map
    .map((line, row) =>
      line
        .map((cell, col) => {
          ...
          const pathValue = path.get(pathKey)?.map((direction) => direction.split(',').map(Number));

          if (pathValue) {
            const hasVerticalDirection = pathValue.some((direction) => direction[0] !== 0);
            const hasHorizontalDirection = pathValue.some((direction) => direction[1] !== 0);
